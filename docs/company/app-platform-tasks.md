...
- [x] 전체 대시보드 KPI 타일 4개(전체·실행중·에러율·p95) + 앱별 요청 스파크라인. 차트 라이브러리
  없이 인라인 SVG — 목록 한 행에 chart.js 를 띄우는 비용이 정보값보다 크다. 좌표는 Go 에서
  계산한다(`sparklinePoints`): 템플릿 산술은 빈 시계열 하나에 0 나누기로 백지 렌더가 된다.
  플랫 시계열도 선을 그린다 — 선이 없는 행은 '조용함'이 아니라 '고장'으로 읽힌다
- [x] 이스케이프 검증 (`company/xss_guard_test.go`) — 주입 시나리오를 손으로 한 번 해보는 대신
  **규칙을 테스트로 고정**했다: (1) `custom/templates` 어느 파일도 `SafeHTML`/`Str2html` 로
  이스케이프를 끄지 않는다, (2) 비상수 `Tr` 키는 우리 어휘이거나 `HasKey` 가드를 거친다 —
  **`Tr` 는 미등록 키를 `template.HTML` 로 그대로 반환**하므로 가드 없는 동적 키는 위장된
  이스케이프 해제다, (3) 라벨 함수는 어떤 입력에도 키 아니면 평문만 돌려준다.
  이 과정에서 `history_table` 의 `.What` 가드 누락을 실제로 찾아 고쳤다
...
- [x] i18n 3단계 — `userError` 가 키+인자를 나른다. 오류는 요청에서 먼 곳에서 생기므로 생성
  시점에는 번역할 수 없고, **보여주는 쪽이 그 사람의 언어로** 번역한다. `Error()` 는 영어
  고정 — 요청 언어에 따라 달라지는 로그는 grep 할 수 없다. 두 청중의 인자를 분리한 이유:
  관리자 문장은 경로·원시 오류를 담는데 그걸 부서 문장에 넣으면 `%!(EXTRA …)` 로 깨지거나
  숨기려던 정보를 그대로 넘긴다 (첫 판에서 실제로 로그에 찍혔다)
- [x] i18n 2단계 완료 — company 템플릿 전체가 로케일 키를 쓴다. 남은 한국어 2줄은 주석 안의 예시
  문구뿐이다(`workspace.tmpl` 의 "이거 고쳐줘", `home_sidebar_top.tmpl` 의 "편집" 설명).
  en-US/ko-KR 양쪽에 등록했고, 한국어 어순이 뒤집히는 곳은 `%[1]d`/`%[2]d` 인덱스를 쓴다
