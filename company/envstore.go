...
func LoadAppEnv(owner, repo string) (map[string]string, int64, error) {
	...
	for name, encrypted := range f.Vars {
		...
		if err != nil {
			...
			return nil, 0, fmt.Errorf("environment variable %q could not be decrypted (was SECRET_KEY changed?)", name)
		}
		out[name] = value
	}
...
func ValidateEnvName(name string) error {
	if name == "" {
		return errors.New("the variable name is empty")
	}
	if len(name) > 128 {
		return fmt.Errorf("the variable name %q is too long", name[:32]+"…")
	}
	if !envNamePattern.MatchString(name) {
		return fmt.Errorf("%q is not a valid variable name — use letters, digits and underscores, starting with a letter", name)
	}
	if isReservedEnvName(name) {
		...
		// bug.
		return fmt.Errorf("%q is reserved by the platform and cannot be set", strings.ToUpper(name))
	}
	return nil
}
...
func SaveAppEnv(owner, repo, actor string, set map[string]string, unset []string) error {
	for name, value := range set {
		...
		if len(value) > maxEnvValueSize {
			return fmt.Errorf("the value for %q is too large (limit %d bytes)", name, maxEnvValueSize)
		}
		if strings.ContainsRune(value, 0) {
			// A NUL terminates the string in the environment block, so the
			// process would silently receive a truncated credential.
			return fmt.Errorf("the value for %q contains a null byte", name)
		}
	}
	...
	if len(f.Vars) > maxEnvVars {
		return fmt.Errorf("an app may have at most %d environment variables", maxEnvVars)
	}
